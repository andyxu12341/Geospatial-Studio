...
import {
  ...
  Map, Copy, Sun, Moon, Trash2, RotateCcw, Clock, History
} from "lucide-react";
...
import { QueryModeCard } from "@/components/QueryModeCard";
...
import { GeoMap, type MapMarker, type GeoMapHandle, type MapPolygon } from "@/components/GeoMap";
...
export default function Index() {
  ...
  const { handleQuery, error: spatialQueryError } = useSpatialQuery({
    geoMapRef,
    gaodeKey,
    baiduKey,
    onResults: (res) => setAreaResults(res)
  });

  const { queryCategory } = useSpatialQueryStore();
  const isAreaQueryMode = appMode === "polygon" && queryCategory === "area";
        ...
        <Tabs value={appMode} onValueChange={(v) => setAppMode(v as "geocoding" | "polygon")} className="mb-4">
          ...
          <div className="flex flex-col md:flex-row gap-4 mt-4">
            ...
            <div className="w-full md:w-[400px] shrink-0 space-y-3 overflow-y-auto pr-1" style={{ maxHeight: "calc(100vh - 120px)" }}>
              ...
              {/* Tab B: Spatial Query */}
              <TabsContent value="polygon" className="mt-0 space-y-3">
                <motion.div
                  key="tab-polygon-panel"
                  initial={{ opacity: 0, x: 8 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.25, ease: "easeOut" }}
                >
                  <AreaQueryPanel mapSource={mapSource} />
                </motion.div>
                <motion.div
                  key="tab-polygon-mode"
                  initial={{ opacity: 0, x: 8 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.25, ease: "easeOut", delay: 0.05 }}
                >
                  <QueryModeCard onQuery={handleQuery} error={spatialQueryError} />
                </motion.div>
              </TabsContent>
            </div>
